<template>
  <div class="parameter_edit">
    <div class="micro_editors_title cl_both">
      <span class="big cl_l">画布尺寸</span>
      <span class="small cl_l">{{matherSetSize.width+'*'+matherSetSize.height}}</span>
    </div>
    <div class="micro_line clear_bottom"></div>
    <div class="micro_editors_title cl_both">
      <span class="big cl_l">图层名称</span>
      <span class="small cl_l">{{selectObj.title}}</span>
    </div>
    ...
    <el-form label-width="75px" label-position="left" size="mini">
      <el-form-item label="坐标" v-show="selectObj.order!=0" class="line_bottom">
        <div>
          <div class="right_item">
            <el-input-number
              v-model="selectObj.realLeft"
              :disabled="selectObj.lockMovementX==true"
              :precision="0"
              @change="updateCanvas("left")"
              controls-position="right"
            ></el-input-number>X
          </div>
          <div class="toggle-lock">
            <i
              :class="{'lock-icon el-icon-lock':selectObj.lockMovementX!=true,'unlock-icon el-icon-unlock':selectObj.lockMovementX==true}"
              @click="toggleLockOrFlip('lockMovementX')"
            ></i>
          </div>
          <div class="right_item">
            <el-input-number
              v-model="selectObj.realTop"
              :precision="0"
              :disabled="selectObj.lockMovementY==true"
              @change="updateCanvas("top")"
              controls-position="right"
            ></el-input-number>Y
          </div>
          <div class="toggle-lock">
            <i
              :class="{'lock-icon el-icon-lock':selectObj.lockMovementY!=true,'unlock-icon el-icon-unlock':selectObj.lockMovementY==true}"
              @click="toggleLockOrFlip('lockMovementY')"
            ></i>
          </div>
        </div>
      </el-form-item>
      ...
      <el-form-item v-show="selectObj.type=='image'&&selectObj.order!=0" label="尺寸">
        <div>
          <div class="right_item">
            <el-input-number
              v-model="selectObj.realWidth"
              controls-position="right"
              :disabled="selectObj.lockScalingY==true"
              :min="1"
              :precision="0"
              @change="updateCanvas("width")"
            ></el-input-number>宽
          </div>
          <div class="toggle-lock"></div>
          <div class="right_item">
            <el-input-number
              v-model="selectObj.realHeight"
              controls-position="right"
              :min="1"
              :disabled="selectObj.lockScalingY==true"
              :precision="0"
              @change="updateCanvas("height")"
            ></el-input-number>高
          </div>
          <div class="toggle-lock">
            <i
              :class="{'lock-icon el-icon-lock':selectObj.lockScalingY!=true,'unlock-icon el-icon-unlock':selectObj.lockScalingY==true}"
              @click="toggleLockOrFlip('lockScalingY')"
            ></i>
          </div>
        </div>
      </el-form-item>
      ...
      <el-form-item label="旋转" v-show="selectObj.order!=0">
        <el-col :span="9" style="text-align:center">
          <el-input-number
            v-model="selectObj.angle"
            controls-position="right"
            :precision="0"
            @change="updateCanvas("angle")"
          ></el-input-number>角度
        </el-col>
      </el-form-item>
      <div class="micro_line" v-show="selectObj.order!=0"></div>
      ...
      <el-form-item label="翻转" v-show="selectObj.type=='image'&&selectObj.order!=0">
        <el-col :span="11" class="flex-box-normal flip-row">
          <i
            class="flip_horizontal el-icon-d-caret"
            @click="toggleLockOrFlip('flipX')"
            title="垂直翻转"
          ></i>
          <i class="flip_vertical el-icon-d-caret" title="水平翻转" @click="toggleLockOrFlip('flipY')"></i>
        </el-col>
      </el-form-item>
      <!-- <div class="micro_line" v-show="selectObj.order!=0"></div> -->
      <!--图片 start~~~~~~~~~~~~~~~~~~-->
    ...
    <el-form label-width="75px" label-position="left" size="mini" v-show="selectObj.type!='image'">
      ...
      <el-form-item label="文字" class="line_bottom">
        <textarea
          ...
          rows="3"
          cols="20"
          v-model="selectObj.text"
          :maxLength="maxLen"
          @input="updateCanvas("text")"
        ></textarea>
      </el-form-item>
      <div class="micro_line"></div>
          ...
          <el-col :span="14" class="flex-box-normal" style="text-align:center">
            <el-button-group>
              ...
              <el-color-picker
                ...
                v-model="selectObj.fill"
                @change="updateCanvas("fill")"
              ></el-color-picker>
              <el-button
                :type="selectObj.fontWeight == 'bold'?'primary':''"
                @click="updateCanvas("fontWeight")"
              >B</el-button>
              <el-button
                :type="selectObj.fontStyle == 'italic'?'primary':''"
                @click="updateCanvas("fontStyle")"
              >I</el-button>
...
export default {
  ...
  computed: {
    maxLen: function() {
      if (
        this.isCopyRightCheck &&
        [3, 4, 5].indexOf(this.selectObj.realType) >= 0
      ) {
        return this.selectObj.realType == 3
          ? 16
          : this.selectObj.realType == 4
          ? 20
          : 10;
      }
      return 10000;
